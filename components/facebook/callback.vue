...
export default class FacebookCallBack extends Vue {
    ...
    async onClickRequest(): Promise<void> {
        await this.$axios
            .get(
                `https://graph.facebook.com/v6.0/oauth/access_token?code=${this.code}&client_id=${this.clientId}&client_secret=${this.clientSecret}&redirect_uri=http://localhost:3000/callback`
            )
            .then((res: any) => {
